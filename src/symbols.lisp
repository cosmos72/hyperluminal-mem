     ...
     (with-package-iterator (iter package :external)
       (loop do
            (multiple-value-bind (found sym) (iter)
              (unless found
                (return))
              (if (position sym start-with)
                  (incf n-syms-head)
                  (progn
                    (incf n-syms-tail)
                    (push sym syms)))))

       (setf n-syms (+ n-syms-head n-syms-tail))

       (unless (= n-syms-head (length start-with))
         (error "HYPERLUMINAL-DB: cannot compile! ~R of the symbols ~S are not present in package ~S"
                (- (length start-with) n-syms-head) start-with (package-name package)))

       (when (and expected-n-symbols
                  (not (eql n-syms expected-n-symbols)))
         (error "HYPERLUMINAL-DB: cannot compile! found ~S external symbols in package ~S, expecting ~S"
                n-syms (package-name package) expected-n-symbols))

       (make-array n-syms :element-type 'string :initial-contents
                   (append start-with
 ...
 *print-pretty* *print-radix* *print-readably* *print-right-margin* *query-io*
 ...
 *trace-output* + ++ +++ - / // /// /= 1+ 1- < <= = > >= abort abs acons acos
 ...
 delete delete-duplicates delete-file delete-if delete-if-not delete-package
 ...
 funcall function function-keywords function-lambda-expression functionp gcd
 ...
 namestring nbutlast nconc next-method-p nintersection ninth
 ...
 number numberp numerator nunion oddp open open-stream-p optimize or otherwise
 ...
 synonym-stream-symbol tagbody tailp tan tanh tenth terpri the third throw
