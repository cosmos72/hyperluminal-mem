...
(defknown %fast-mwrite/8
    ;;arg-types
    ((unsigned-byte 64) fixnum fixnum (member 1 2 4 8) (signed-byte 32))
    ;;result-type
    (values)
...
(sb-c:define-vop (%fast-mread/8)
  (:policy :fast-safe)
  (:translate %fast-mread/8)
  ...
  (:generator 1
   (sb-assem:inst mov r (sb-vm::make-ea :qword :base base :index index
					:scale scale :disp disp))))


(sb-c:define-vop (%fast-mwrite/8)
  (:policy :fast-safe)
  (:translate %fast-mwrite/8)
  ...
  (:generator 1
   (sb-assem:inst mov (sb-vm::make-ea :qword :base base :index index
				      :scale scale :disp disp) value)))


(defmacro fast-mread/8 (base index
  ...
  (let ((scale (/ (check-compile-constant scale) +fixnum-zero-mask+1+))
    ...
    `(%fast-mread/8 ,base ,index ,scale ,disp)))

(defmacro fast-mwrite/8 (value base index
			 &key (scale +fixnum-zero-mask+1+) (disp 0))
  (let ((scale (/ (check-compile-constant scale) +fixnum-zero-mask+1+))
	(disp  (check-compile-constant disp)))
    (check-type scale (member 1 2 4 8))
    `(%fast-mwrite/8 ,value ,base ,index ,scale ,disp)))
